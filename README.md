...
It is a separate React + Vite + TypeScript + Tailwind app with a Zustand
investigation store. The build writes static assets to `docs/workstation/` so
GitHub Pages can eventually serve it at `/workstation/`.

The workstation reads the existing public case-file index from
...
Phase 3D turns Case Mode into a linked evidence canvas. Chart hover/select state
drives the PointInspector, ComparatorInspector, EvidenceNarrativePanel, and
selected-window readouts. Comparator focus emphasizes the relevant existing
case-file evidence, and the right column keeps next checks and uncertainty notes
visible without adding new inference.

Phase 3E adds Aladin Lite sky context to Case Mode and links the GitHub Pages
demo page to the built workstation. The sky panel centers external sky imagery
on the case-file coordinates, marks the object position, and keeps
catalog-context status and caveats visible. The workstation remains a static
GitHub Pages artifact; Aladin Lite imagery loads from external services at
browser runtime and does not affect the Python pipeline or offline tests.

Phase 3F polishes the workstation into a more coherent analyst surface. Queue
glyph selection, CaseHeader status chrome, chart focus, linked point guides,
evidence focus, and calm missing states now use a shared visual language. This
